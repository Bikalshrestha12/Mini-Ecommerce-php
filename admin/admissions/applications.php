...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_status') {
        ...
        if ($application_id && in_array($status, $validStatuses)) {
            $stmt = $pdo->prepare("UPDATE admission_applications SET status = ?, updated_at = datetime('now') WHERE application_id = ?");
    ...
    if ($action === 'delete') {
        $application_id = $_POST['application_id'] ?? '';
        if ($application_id) {
            $stmt = $pdo->prepare("DELETE FROM admission_applications WHERE application_id = ?");
...
$sql = "SELECT a.*, p.title as program_title FROM admission_applications a LEFT JOIN admission_programs p ON a.program_id = p.program_id";
$params = [];
if ($programFilter) {
...
$programs = $pdo->query("SELECT program_id, title FROM admission_programs ORDER BY title")->fetchAll();
$statuses = ['Pending', 'Reviewed', 'Accepted', 'Rejected'];
?>

<div class="d-flex justify-content-between align-items-center mb-3">
    <h5 class="mb-0 fw-bold">Admission Applications</h5>
    <div>
        <a href="index.php" class="btn btn-outline-secondary btn-sm"><i class="fas fa-arrow-left"></i> Back to Programs</a>
    </div>
</div>
...
<div class="card shadow-sm mb-3">
    <div class="card-body py-2">
        <form method="get" class="row g-2 align-items-center">
            <div class="col-auto">
                <label class="form-label mb-0 fw-bold">Filter by Program:</label>
            </div>
            <div class="col-auto">
                <select name="program_id" class="form-select form-select-sm" onchange="this.form.submit()">
                    <option value="">All Programs</option>
                    <?php foreach ($programs as $p): ?>
                        <option value="<?= $p['program_id'] ?>" <?= $programFilter == $p['program_id'] ? 'selected' : '' ?>><?= h($p['title']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <table class="table table-hover datatable">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?= h($a['phone'] ?? '-') ?></td>
                        <td><?= h($a['program_title'] ?? '-') ?></td>
                        <td>
                            <?php if ($a['documents']): ?>
                                <a href="<?= APP_URL . '/' . h($a['documents']) ?>" class="btn btn-sm btn-outline-info" target="_blank">
...
function viewApplication(data) {
    ...
    html += '<tr><th>Program</th><td>' + h(data.program_title || '-') + '</td></tr>';
    ...
    html += '<tr><th>Address</th><td>' + h(data.address || 'N/A') + '</td></tr>';
    html += '<tr><th>Date of Birth</th><td>' + (data.date_of_birth ? h(data.date_of_birth) : 'N/A') + '</td></tr>';
    html += '<tr><th>Gender</th><td>' + h(data.gender || 'N/A') + '</td></tr>';
    html += '<tr><th>Previous Education</th><td>' + h(data.previous_education || 'N/A') + '</td></tr>';
    html += '<tr><th>Documents</th><td>' + (data.documents ? '<a href="<?= APP_URL ?>/' + h(data.documents) + '" target="_blank" class="btn btn-sm btn-outline-info"><i class="fas fa-download"></i> Download</a>' : 'N/A') + '</td></tr>';
    html += '</table>';
